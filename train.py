...
from DensenetSiamese import DensenetSiameseNetwork
...
from utils import prep_dataset, prep_datasetV2, check_dataset, show_plot, TrainDatasetV3, TrainDatasetV4
...
def train_fn(X_train,X_val, args):
    ...
    if args.weakly_supervised == True:
        train_dataset=TrainDatasetV4(X_train, args.image_file_folder, args.dataframe_path, transform = transformV1)
        val_dataset=TrainDatasetV4(X_val, args.image_file_folder, args.dataframe_path, transform = transformV1)
    else:
        train_dataset=TrainDatasetV3(X_train, args.image_file_folder, transform = transformV1)
        val_dataset=TrainDatasetV3(X_val, args.image_file_folder, transform = transformV1)
    ...
    for epoch in range(args.num_epochs):
        ...
        for idx, (img0, img1, label) in batch:
            img0 = img0.to(device)
            img1 = img1.to(device)
        ...
        for idx, (val_img0, val_img1, val_label) in val_batch:
            val_img0 = val_img0.to(device)
            val_img1 = val_img1.to(device)
            val_label = val_label.to(device)
            
            with torch.no_grad():
                val_output1, val_output2 = model(val_img0, val_img1)
